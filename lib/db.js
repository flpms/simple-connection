...
class DB {
  ...
  constructor(config, mongoClient) {
    ...
    const protocol = config.protocol ? `${config.protocol}` : 'mongodb://';

    this.url = `${protocol}${userAndPass(config.username, config.password)
    }${config.server}${port}/${config.database_name}${(config.query_params || '')}`;
